package com.projecturanus.betterp2p_plusplus.client.gui

import com.projecturanus.betterp2p_plusplus.BetterP2P
import com.projecturanus.betterp2p_plusplus.MODID
import com.projecturanus.betterp2p_plusplus.capability.MemoryInfo
import com.projecturanus.betterp2p_plusplus.client.ClientCache
import com.projecturanus.betterp2p_plusplus.client.TextureBound
import com.projecturanus.betterp2p_plusplus.client.gui.widget.WidgetP2PDevice
import com.projecturanus.betterp2p_plusplus.client.gui.widget.WidgetScrollBar
import com.projecturanus.betterp2p_plusplus.item.BetterMemoryCardModes
import com.projecturanus.betterp2p_plusplus.network.*
...
class GuiAdvancedMemoryCard(msg: S2CListP2P) : GuiScreen(), TextureBound {
    ...
    init {
        ...
        for (i in 0..4) {
            list += WidgetP2PDevice(::selectedInfo, ::mode,  { sortedInfo.getOrNull(i) }, 0, 0)
        }
    ...
    override fun initGui() {
        ...
        for (i in 0..4) {
            ...
            widgetDevices[i].infoSupplier = { sortedInfo.getOrNull(i + scrollBar.currentScroll) }
        }

        searchField = GuiTextField(0, fontRenderer, guiLeft + 9, guiTop + 5, rowWidth, 10)
    ...
    fun sortInfo() {
        val typedText = if (::searchField.isInitialized) searchField.text.trim() else ""
        var filteredInfos = if (typedText.isNotEmpty()) {
            infos.filter { it.displayName.contains(typedText, ignoreCase = true) }
        } else {
            infos
        }

        sortedInfo = filteredInfos.sortedBy {
        ...
        if (::scrollBar.isInitialized) {
            scrollBar.setRange(0, sortedInfo.size.coerceIn(0..(sortedInfo.size-5).coerceAtLeast(0)), 23)
        }
    }
